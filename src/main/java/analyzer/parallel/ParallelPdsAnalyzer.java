...
//        JavaPairRDD<Integer, int[]> deltaPair3 = sc.parallelize(ruleSet3).mapToPair(deltaPairFunc);
        ...
        while (true) {
            ...
            transPair = transPair.subtract(matchedTrans).subtract(matchedTrans2).union(newTrans).union(newTrans2).distinct().cache(); // TODO distinct
            relPair = relPair.union(matchedTrans).union(matchedTrans2).distinct().cache(); // TODO distinct


            deltaPair4 = deltaPair4.union(newDeltaPair).distinct().cache(); // TODO distinct
